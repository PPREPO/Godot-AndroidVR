...
var testing_cfg:Dictionary = {"head_tilt" = false}
...
func gyroscope_handled() -> void: #gyro handled in androidVR
	...
	if gyroscope_direction != Vector3.ZERO:
		head_controls.rotate_x(gyroscope_direction.x * gyro_sensitivity * Global.head_x_direction) #heads look up-down
		rotate_y((gyroscope_direction.y * gyro_sensitivity * Global.head_y_direction)/2) #heads look left-right
		if Global.head_tilt:
